...
type webStaticGoals struct {
	...
	formatHtmlDuration time.Duration
}
...
func (me *webStaticGoals) run() {
	...
	log.Printf("Total duration of formatHtml() calls: %v", me.formatHtmlDuration)
}
...
func (me *webStaticGoals) generateGoal(lang language.Tag, goalsPath string, goal db_objects.GoalRow) {
	...
	gophers.AssertError(os.MkdirAll(path, file_mode.USER_RWX))
	var posts = me.db.getGoalPostHeaders(goal.Id, false, lang)
...
func (webStaticGoals) getPublicUrl(row *db_objects.GoalPostRow, lang language.Tag) string {
	return webStaticGoals{}.getPublicBaseUrl() + webStaticGoals{}.getLanguagePath(lang) + "/personal-goals/" +
...
func (me *webStaticGoals) formatHtml(text string) string {
	...
	me.formatHtmlDuration += time.Since(start)
	return result
}
